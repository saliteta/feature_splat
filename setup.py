...
__version__ = None
exec(open("feature_splat/version.py", "r").read())
...
def get_extensions():
    ...
    extensions_dir_v2 = osp.join("feature_splat", "cuda", "csrc")
    ...
    glm_path = os.path.join(current_dir, "feature_splat", "cuda", "csrc", "third_party", "glm")
...
setup(
    name="feature_splat",
    version=__version__,
    description=" Python package for differentiable rasterization of gaussians, originate from gsplat from nerfstudio, however, we modified it",
    keywords="gaussian, splatting, cuda",
    url=URL,
    download_url=f"{URL}/archive/feature_splat-{__version__}.tar.gz",
    python_requires=">=3.7",
    install_requires=[
        "ninja",
        "numpy",
    ...
    extras_require={
        # dev dependencies. Install them by `pip install feature_splat[dev]`
        "dev": [
