...
class _RasterizeToPixels(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.absgrad = absgrad
        ctx.color_channel = colors.shape[-1]
    ...
    def backward(
        ...
        absgrad = ctx.absgrad
        color_channel = ctx.color_channel

        v_colors= _make_lazy_cuda_func("rasterize_to_pixels_bwd")(
            means2d,
            conics,
            opacities,
            backgrounds,
        ...
        return (
                None,  # v_means2d modified version does not need gradient
                None,  # v_conics modified version does not need gradient
                v_colors, 
                None,  # v_opacities modified version does not need gradient
                v_backgrounds,
                None,  # width, does not need a gradient
                None,  # height, does not need a gradient
                None,  # tile_size, does not need a gradient
                None,  # isect_offsets, does not need a gradient
                None,  # flatten_ids, does not need a gradient
                None   # absgrad, non-tensor parameter
        )
