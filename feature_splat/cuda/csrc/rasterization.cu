...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> rasterize_to_pixels_fwd_tensor(
    ...
    case 512:
        ...
        break;
    case 1024:
        rasterize_to_pixels_fwd_kernel<1024><<<blocks, threads, 0, stream>>>(
            C, N, n_isects, packed, (float2 *)means2d.data_ptr<float>(),
            (float3 *)conics.data_ptr<float>(), colors.data_ptr<float>(),
            opacities.data_ptr<float>(),
            backgrounds.has_value() ? backgrounds.value().data_ptr<float>() : nullptr,
    ...
    template <uint32_t COLOR_DIM>
    __global__ void rasterize_to_pixels_features_bwd_kernel(
        const uint32_t C, const uint32_t N, const uint32_t n_isects, const bool packed,
        // fwd inputs
        const float2 *__restrict__ means2d,    // [C, N, 2] or [nnz, 2]
        const float3 *__restrict__ conics,     // [C, N, 3] or [nnz, 3]
        ...
        // This last id seems like the ID of the last gaussian's that we need to render. Since we will render until the alpha is accumulate to 1/255 
        // grad outputs
        ...
        uint32_t camera_id = block.group_index().x;
        uint32_t tile_id = block.group_index().y * tile_width + block.group_index().z;
        uint32_t i = block.group_index().y * tile_size + block.thread_index().y; // in a specific tile, x pixel
        uint32_t j = block.group_index().z * tile_size + block.thread_index().x; // in a specifc tile, y pixel

        tile_offsets += camera_id * tile_height * tile_width; // try to move to that specifc tile first move a camera
        render_alphas += camera_id * image_height * image_width;  
        last_ids += camera_id * image_height * image_width;
        v_render_colors += camera_id * image_height * image_width * COLOR_DIM; // move to specifc color entry
        if (backgrounds != nullptr) {
            backgrounds += camera_id * COLOR_DIM;
        }
        ...
        // first collect gaussians between range.x and range.y in batches
        // which gaussians to look through in this tile
        int32_t range_start = tile_offsets[tile_id]; // move to the tile where we should start
        int32_t range_end =
            (camera_id == C - 1) && (tile_id == tile_width * tile_height - 1)
                ? n_isects
                : tile_offsets[tile_id + 1];
        // we will deal with Gaussian's processing for one tile. That means, one block deal with one tile
        const uint32_t block_size = block.size();
        const uint32_t num_batches = (range_end - range_start + block_size - 1) / block_size; 
        // each Gaussian that in this block will be processed, but we need to add paddings
        // which futher implites, ine thread deal with one Gaussian. We have (tileoffset[n+1] - tileoffset[n]) 
        // unit to deal with

        // MAX BLOCK SIZE IS 16 * 16 block-wise operation
        __shared__ int32_t id_batch[MAX_BLOCK_SIZE];
        __shared__ float3 xy_opacity_batch[MAX_BLOCK_SIZE];
        __shared__ float3 conic_batch[MAX_BLOCK_SIZE];
        ...
        float T = T_final;
        // I think this means background contribution 
        // the contribution from gaussians behind the current one
        
        // if inside, bin_final is last_ids[pix_id], otherwise zero
        const int32_t bin_final = inside ? last_ids[pix_id] : 0;

        // df/d_out for this pixel, the one we really care about for this thread
        ...
        // theoratically, the upper bound of the overall GaussianID[idx] warp_bin_final 
        // always larger then the idx

        // FOR ALL GAUSSIAN PROJECTED IN A TILE
        for (uint32_t b = 0; b < num_batches; ++b) {
            ...
            // each block, we will use all of thread to process, each thread process a gaussian
            const int32_t batch_end = range_end - 1 - block_size * b; 
            const int32_t batch_size = min(block_size, batch_end + 1 - range_start);
            const int32_t idx = batch_end - tr;
            if (idx >= range_start) {
                int32_t g = flatten_ids[idx]; // flatten index in [C * N] or [nnz]
                id_batch[tr] = g; // gaussian ID for this batch
                const float2 xy = means2d[g]; // 
                const float opac = opacities[g];
                xy_opacity_batch[tr] = {xy.x, xy.y, opac};
                conic_batch[tr] = conics[g];
            } 
            // load gaussian ID, xy, and opac to shared memory
            ...
            // before this for loop, we load all related gaussian to shared memory
            // conic_batch, id_batch, xy_opacity_batch[means2dx, means2dy, alpha]
            // skip some of the gaussian that has minimal contribution to the rendering process
            for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t) {
                // for this batch, let us start with the last Gaussian that affact this tile
                bool valid = inside;
                ...
                if (valid) {
                    ...
                    // for current frame, which pixel I want to check
                    float sigma =
                        0.5f * (conic.x * delta.x * delta.x + conic.z * delta.y * delta.y) +
                        conic.y * delta.x * delta.y;
                    vis = __expf(-sigma);
                    alpha = min(0.999f, opac * vis); // bledning Gaussian Scale and alpha
                    if (sigma < 0.f || alpha < 1.f / 255.f) {
                        valid = false;
                    }
                }
                ...
                // v_rgb_local must be the gradient newly calculate for 
                // id_batch[t] not others
                if (warp.thread_rank() == 0) {
                    int32_t g = id_batch[t]; // flatten index in [C * N] or [nnz]
                    // for the current gaussian g
                    // for this g, we need to update its v_rgb_ptr
...
rasterize_to_pixels_bwd_tensor(
    ...
    // image size
    const uint32_t image_width, const uint32_t image_height, const uint32_t tile_size, const uint32_t channel_number, 
    // intersections
    const torch::Tensor &tile_offsets, // [C, tile_height, tile_width]
    const torch::Tensor &flatten_ids,  // [n_isects]
    // forward outputs
    ...
    CHECK_INPUT(means2d);
    CHECK_INPUT(conics);
    // CHECK_INPUT(colors); // no need to pass in anymore
    CHECK_INPUT(opacities);
    CHECK_INPUT(tile_offsets);
    CHECK_INPUT(flatten_ids);
    ...
    uint32_t N = packed ? 0 : means2d.size(1); // number of gaussians
    uint32_t n_isects = flatten_ids.size(0);
    uint32_t COLOR_DIM = channel_number; // no need to pass in anymore 
    uint32_t tile_height = tile_offsets.size(1);
    uint32_t tile_width = tile_offsets.size(2);
    ...
    torch::Tensor v_colors = torch::empty({means2d.size(0), COLOR_DIM},   
                             means2d.options().dtype(torch::kFloat32));
    ...
    if (n_isects) {
        ...
        switch (COLOR_DIM) {
        case 1:
            rasterize_to_pixels_features_bwd_kernel<1><<<blocks, threads, 0, stream>>>(
                C, N, n_isects, packed, (float2 *)means2d.data_ptr<float>(),
                (float3 *)conics.data_ptr<float>(), opacities.data_ptr<float>(),
            ...
            break;
        case 2:
            rasterize_to_pixels_features_bwd_kernel<2><<<blocks, threads, 0, stream>>>(
                C, N, n_isects, packed, (float2 *)means2d.data_ptr<float>(),
                (float3 *)conics.data_ptr<float>(), opacities.data_ptr<float>(),
                backgrounds.has_value() ? backgrounds.value().data_ptr<float>()
                                        : nullptr,
            ...
            break;
        case 3:
            rasterize_to_pixels_features_bwd_kernel<3><<<blocks, threads, 0, stream>>>(
                C, N, n_isects, packed, (float2 *)means2d.data_ptr<float>(),
                (float3 *)conics.data_ptr<float>(), opacities.data_ptr<float>(),
                backgrounds.has_value() ? backgrounds.value().data_ptr<float>()
                                        : nullptr,
            ...
            break;
        case 4:
            rasterize_to_pixels_features_bwd_kernel<4><<<blocks, threads, 0, stream>>>(
                C, N, n_isects, packed, (float2 *)means2d.data_ptr<float>(),
                (float3 *)conics.data_ptr<float>(), opacities.data_ptr<float>(),
                backgrounds.has_value() ? backgrounds.value().data_ptr<float>()
                                        : nullptr,
            ...
            break;
        case 8:
            rasterize_to_pixels_features_bwd_kernel<8><<<blocks, threads, 0, stream>>>(
                C, N, n_isects, packed, (float2 *)means2d.data_ptr<float>(),
                (float3 *)conics.data_ptr<float>(), opacities.data_ptr<float>(),
                backgrounds.has_value() ? backgrounds.value().data_ptr<float>()
                                        : nullptr,
            ...
            break;
        case 16:
            rasterize_to_pixels_features_bwd_kernel<16><<<blocks, threads, 0, stream>>>(
                C, N, n_isects, packed, (float2 *)means2d.data_ptr<float>(),
                (float3 *)conics.data_ptr<float>(),opacities.data_ptr<float>(),
                backgrounds.has_value() ? backgrounds.value().data_ptr<float>()
                                        : nullptr,
            ...
            break;
        case 32:
            rasterize_to_pixels_features_bwd_kernel<32><<<blocks, threads, 0, stream>>>(
                C, N, n_isects, packed, (float2 *)means2d.data_ptr<float>(),
                (float3 *)conics.data_ptr<float>(), opacities.data_ptr<float>(),
                backgrounds.has_value() ? backgrounds.value().data_ptr<float>()
                                        : nullptr,
            ...
            break;
        case 1024:
            rasterize_to_pixels_features_bwd_kernel<1024><<<blocks, threads, 0, stream>>>(
                C, N, n_isects, packed, 
